...
export async function generateCalendarFromAnswers(
  answers: QuizAnswers,
  ...
  const {
    ...
    pestSchedule, sqft, allergies, allergiesDetails,
    crawlSpace, crawlSpaceSealed, landscaping, yearBuilt: answersYearBuilt,
  } = answers;
  ...
  const allergiesText    = allergies === "yes"
    ? `Yes${allergiesDetails ? ` — ${allergiesDetails}` : ""}` : "No";
  const crawlSpaceText   = crawlSpace === "yes"
    ? `Yes (${CRAWL_SEALED_LABELS[crawlSpaceSealed ?? ""] ?? "status unknown"})` : "No";

  const currentYear = new Date().getFullYear();
  const resolvedYearBuilt = homeProfile?.yearBuilt ?? answersYearBuilt ?? null;

  let homeAgeContext: string;
  if (resolvedYearBuilt) {
    const age = currentYear - resolvedYearBuilt;
    homeAgeContext = `Home built in ${resolvedYearBuilt} (${age} years old — use this precise age for lifespan calculations)`;
  } else {
  ...
  const forecastContext = buildForecastContext(resolvedYearBuilt, currentYear, roofType);
...
big_ticket_alerts: 3 items max. one_time_tasks: 3 items max.`;
  ...
  const message = await anthropic.messages.create({
    ...
    max_tokens: 8192,
    system: systemPrompt,
    messages: [{ role: "user", content: "Generate my personalized home maintenance calendar." }],
  });

  if (message.stop_reason === "max_tokens") {
    throw new Error("AI response truncated — hit max_tokens");
  }
